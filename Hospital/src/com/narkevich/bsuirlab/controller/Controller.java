package com.narkevich.bsuirlab.controller;


import com.narkevich.bsuirlab.model.service.*;
import com.narkevich.bsuirlab.view.View;
...
public class Controller {
            ...
            switch (choice){
                ...
                case 7:
                    ...
                    try{
                        ...
                        view.showDepartmentAndDoctorMismatch();
                    }catch (NumberFormatException e){
                        view.showExceptionInfo();
                    }
                    break;
    ...
    private void Start(Scanner userKey){
        boolean wasSuccessful = false;
        while (!wasSuccessful) {
            wasSuccessful = true;
            try {
                view.showHospitalEnter();
                String country;
                view.showCountry();
                country = userKey.nextLine();
                if (country.trim().isEmpty()) {
                    throw new NullStringException();
                }
                String city;
                view.showCity();
                city = userKey.nextLine();
                if (city.trim().isEmpty()) {
                    throw new NullStringException();
                }
                String address;
                view.showAddress();
                address = userKey.nextLine();
                if (address.trim().isEmpty()) {
                    throw new NullStringException();
                }
                String name;
                view.showHospitalName();
                name = userKey.nextLine();
                if (name.trim().isEmpty()) {
                    throw new NullStringException();
                }
                hospital.setInfo(country, city, address, name);
            } catch (NullStringException e) {
                view.showExceptionInfo();
                wasSuccessful = false;
            }
